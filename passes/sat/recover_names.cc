...
struct RecoverModuleWorker {
    ...
    void group_classes(dict<equiv_cls_t, std::pair<pool<IdBit>, pool<IdBit>>> &cls2bits, bool is_gate)
    {
        ...
        for (auto pair : bit2cls) {
            ...
            (is_gate ? cls2bits[pair.second].second : cls2bits[pair.second].first).insert(pair.first);
        }
    }
    ...
    void do_rename(Module *gold, const dict<IdBit, IdBit> &gate2gold, const pool<IdString> &buffer_types)
    {
        ...
        // gate sigbit --> (new name, gold wire)
        dict<SigBit, std::pair<IdBit, Wire*>> rename_map;
        for (auto pair : gate2gold) {
            SigBit gate_bit = flat2orig.at(pair.first);
            Wire *gold_wire = gold->wire(pair.second.name);
            ...
            if (root2buffered.count(gate_bit)) {
                ...
                for (auto buf_bit : root2buffered.at(gate_bit)) {
                    std::string buf_name_str = stringf("%s$buf$%d", pair.second.name.c_str(), ++buf_idx);
                    if (buf_name_str[0] == '\\')
                        buf_name_str[0] = '$';
                    rename_map[buf_bit] = std::make_pair(IdBit(IdString(buf_name_str), pair.second.bit), gold_wire);
                }
            }
        }
        ...
        for (auto rule : rename_map) {
            ...
            IdBit new_name = rule.second.first;
            int dup_idx = 0;
            while (mod->wire(new_name.name) && !unused_bits.count(SigBit(mod->wire(new_name.name), new_name.bit))) {
                std::string new_name_str = stringf("%s$dup$%d", rule.second.first.name.c_str(), ++dup_idx);
                if (new_name_str[0] == '\\')
                    new_name_str[0] = '$';
...
struct RecoverNamesWorker {
    ...
    // Mapping from bit to (candidate) equivalence classes
    dict<equiv_cls_t, std::pair<pool<IdBit>, pool<IdBit>>> cls2bits; 
    ...
    bool prove_equiv(RecoverModuleWorker &gold_worker, RecoverModuleWorker &gate_worker,
        ...
        // Prove equivalence
        return !ez->solve(ez->NOT(ez->IFF(gold_var, gate_var)));
    }
    ...
    void analyse_mod(Module *gate_mod)
    {
        ...
        for (auto &cls : cls2bits) {
            if (cls.second.second.empty())
                continue;
            int depth = 0;
            for (IdBit gate_bit : cls.second.second) {
                if (!gate_worker.bit2depth.count(gate_bit))
                    continue;
                depth = std::max(depth, gate_worker.bit2depth.at(gate_bit));
            }
        ...
        for (auto cls : cls_depth) {
            ...
            for (IdBit gold_bit : gold_bits) {
                for (IdBit gate_bit : gate_bits) {
                    if (solved_gate.count(gate_bit))
                        continue;
                    log_debug("   attempting to prove %s[%d] == %s[%d]\n", log_id(gold_bit.name), gold_bit.bit, log_id(gate_bit.name), gate_bit.bit);
                    if (!prove_equiv(gold_worker, gate_worker, gold_anchors, gate_anchors, gold_bit, gate_bit))
                        continue;
                    log_debug("       success!\n");
                    // Success!
                    gate2gold[gate_bit] = gold_bit;
                    gold_anchors[gold_bit] = gate_bit;
                    gate_anchors[gate_bit] = gold_bit;
                    solved_gate.insert(gate_bit);
                }
                // All solved...
